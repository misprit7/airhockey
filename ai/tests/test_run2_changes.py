...
def test_drive_income_pays_paddle_speed_toward_a_held_puck():
    ...
    paid = [float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, pad_x=0.5, pad_y=0.20 + 0.04 * k))[0])
            for k in range(1, 3)]
    assert paid == [pytest.approx(R.DRIVE_PAY_MAX), 0.0]
    # backing away, or moving sideways, pays nothing
    sh, obs = held_shaper()
    sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, pad_x=0.5, pad_y=0.24))
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, pad_x=0.5, pad_y=0.20))[0]) == 0.0
    sh, obs = held_shaper()
    sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, pad_x=0.5, pad_y=0.20))
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, pad_x=0.54, pad_y=0.20))[0]) == 0.0
    # and nothing without a hold first
    sh = _shaper(drive_weight=0.5, patience_s=1.5, control_gate=True)
    obs = np.zeros((1, 22), dtype=np.float32)
    fast = _info(0.5, 0.6, 0.0, -2.0)
    sh.reset(obs, info=fast); sh.compute(obs, np.zeros(1), info=fast)
    sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, pad_x=0.5, pad_y=0.20))
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, pad_x=0.5, pad_y=0.24))[0]) == 0.0
