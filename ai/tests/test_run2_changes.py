...
def test_patience_beats_the_discount_by_design():
    ...
    assert kw["overstay_cost"] == 0.0 and kw["windup_income"] == 0.0 and kw["drive_weight"] == 0.25
    assert kw["patience_floor"] == 0.05 and kw["trap_reward"] == 0.0
...
def test_shot_clock_runs_on_time_on_side_whatever_the_puck_does():
    """3.4: past SHOT_CLOCK_S on our side every step costs -- a slow puck,
    a fast one, the paddle near or far -- and nothing before it."""
    sh = _shaper(overstay_cost=0.5, patience_s=2.0)
    obs = np.zeros((1, 22), dtype=np.float32)
    fast = _info(0.5, 0.6, 0.0, -2.0, t_side=0.1)
    sh.reset(obs, info=fast); sh.compute(obs, np.zeros(1), info=fast)
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.36, 0.0, 0.0, t_side=2.9))[0]) == 0.0
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.36, 0.0, 0.0, t_side=3.1))[0]) == pytest.approx(-0.5)
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.60, 1.0, 1.0, t_side=3.2))[0]) == pytest.approx(-0.5)
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 0.40, 0.0, 0.0, t_side=3.3, pad_x=0.1, pad_y=0.1))[0]) == pytest.approx(-0.5)
    assert sh.stats["overstay_steps"] == 3
    # over the line, no clock
    assert float(sh.compute(obs, np.zeros(1), info=_info(0.5, 1.5, 0.0, 1.0, t_side=0.0))[0]) == 0.0
